...
> 很多面试官会问"为什么是15"，这里有个关键误解！

**核心答案**：默认值其实是 **6**，不是15！

但15确实是个特殊数字，它来自**对象头的年龄字段**：
...
**为什么是4位？**
- 对象头空间有限，4位足够表示0-15的年龄
- 实际生产中，年龄超过6的对象很少（说明是长寿对象）
...
1. 大量线上数据分析：
   - 98%的对象在第一次Minor GC时就会死亡
   - 年龄达到2的对象，80%会成为长寿对象
   - 年龄达到4的对象，95%会成为长寿对象

2. 权衡点：
   - 阈值太小 → 对象频繁晋升老年代 → Full GC增加
   - 阈值太大 → Survivor区压力增大 → Minor GC时间变长
...
# 示例输出：
# Desired survivor size 5242880 bytes, threshold 6 (max 6)
# - age   1:    12345678 bytes
# - age   2:     2345678 bytes
# - age   3:      345678 bytes
# - age   4:       45678 bytes
```
...
```bash
# JMX连接后调用
com.sun.management:type=HotSpotDiagnostic.setVMOption("MaxTenuringThreshold", 15)

# 或者使用jinfo
jinfo -flag MaxTenuringThreshold=<value> <pid>
```
...
```bash
# 调整前：频繁Full GC
java -Xms4g -Xmx4g -XX:MaxTenuringThreshold=6 -XX:+UseG1GC \
     -Xlog:gc*:gc.log

# 查看日志发现：大量年龄2-4的对象晋升老年代

# 调整后：
java -Xms4g -Xmx4g -XX:MaxTenuringThreshold=10 -XX:+UseG1GC \
     -XX:+PrintTenuringDistribution -Xlog:gc*:gc.log
